...
- 2026-08-23：保留短计时窗口推演。双轨图给出一个已经定义好的错误窗口，推演则要求学习者先预测、再自行选择同一观察者的边界，并在完成事件缺失时拒绝结果；认识增量是“重建并否决测量合同”，不是再次播放异步动画。移除推演不影响正文知识链，因此它保持从属而非成为章节目录。
- 2026-08-23：最终连续阅读复查通过。正文仍只有五次认知转折；四个图示分别位于窗口、异步、warm-up 和分布概念形成后；计时推演与迁移验收均在完整正文之后；00→01 导航延续“完成事件内部是什么”的最后一问，没有插入新的总结卡片。
- 2026-08-23：视觉验收证据由用户对完整正文与四类图示的页面体验确认，以及新增实践/导航的 DOM、键盘、窄屏 CSS 和 reduced-motion 自动检查共同组成。Browser 连接在最终复查时仍无可用实例，因此没有声称新增一轮自动截图检查；补充修正了 textarea/select 字体继承与移动端实践表格内部滚动。
