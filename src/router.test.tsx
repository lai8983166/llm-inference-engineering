...
describe('minimal course routes', () => {
  ...
  it('renders chapter five with the scheduling manifest and no unpublished next chapter', () => {
    ...
    expect(within(screen.getByRole('navigation', { name: '章节导航' })).getByRole('link', { name: /下一章 · 06.*过载的成本由谁承担/ })).toHaveAttribute('href', '/chapters/overload')
  })

  it('renders chapter six with the overload manifest and no unpublished next chapter', () => {
    renderRoute('/chapters/overload')
    expect(screen.getByRole('heading', { level: 1, name: /过载的成本\s*由谁承担/ })).toBeInTheDocument()
    expect(screen.getByRole('complementary', { name: '第 06 章固定过载输入' })).toBeInTheDocument()
    expect(screen.getByRole('navigation', { name: '本章内容' })).toBeInTheDocument()
    expect(document.querySelectorAll('.chapter-prose > h2')).toHaveLength(5)
    expect(document.querySelectorAll('.chapter-prose')).toHaveLength(1)
    expect(screen.getAllByRole('figure')).toHaveLength(2)
    const finalBodyHeading = screen.getByRole('heading', { level: 2, name: '没有免费方向，只有服务承诺' })
    const practiceHeading = screen.getByRole('heading', { level: 2, name: '先归因拒绝，再重建裁决' })
    expect(finalBodyHeading.compareDocumentPosition(practiceHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
  ...
  it('renders chapter seven with the termination manifest and no unpublished next chapter', () => {
    ...
    expect(document.querySelectorAll('.chapter-prose')).toHaveLength(1)
    expect(screen.getAllByRole('figure')).toHaveLength(2)
    const finalBodyHeading = screen.getByRole('heading', { level: 2, name: '无泄漏合同与可观测的终点' })
    const practiceHeading = screen.getByRole('heading', { level: 2, name: '先判断释放了什么，再重建清理' })
    expect(finalBodyHeading.compareDocumentPosition(practiceHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    expect(screen.getByRole('heading', { level: 2, name: '用新注入验收终止账本' })).toBeInTheDocument()
    expect(screen.queryByText(/学习进度|掌握率/)).not.toBeInTheDocument()
    ...
    expect(within(screen.getByRole('navigation', { name: '章节导航' })).getByRole('link', { name: /下一章 · 08.*均值会说谎，事件不会/ })).toHaveAttribute('href', '/chapters/metrics')
  })

  it('renders chapter eight with the metrics manifest and no unpublished next chapter', () => {
    renderRoute('/chapters/metrics')
    expect(screen.getByRole('heading', { level: 1, name: /均值会说谎，\s*事件不会/ })).toBeInTheDocument()
    expect(screen.getByRole('complementary', { name: '第 08 章固定指标输入' })).toBeInTheDocument()
    expect(screen.getByRole('navigation', { name: '本章内容' })).toBeInTheDocument()
    expect(document.querySelectorAll('.chapter-prose > h2')).toHaveLength(5)
    expect(document.querySelectorAll('.chapter-prose')).toHaveLength(1)
    expect(screen.getAllByRole('figure')).toHaveLength(2)
    const finalBodyHeading = screen.getByRole('heading', { level: 2, name: '聚合链要能走回头路' })
    const practiceHeading = screen.getByRole('heading', { level: 2, name: '先找出藏起来的指标，再重建聚合链' })
    expect(finalBodyHeading.compareDocumentPosition(practiceHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    expect(screen.getByRole('heading', { level: 2, name: '用新负载验收指标口径' })).toBeInTheDocument()
    expect(screen.queryByText(/学习进度|掌握率/)).not.toBeInTheDocument()
    expect(within(screen.getByRole('navigation', { name: '章节导航' })).getByRole('link', { name: /上一章 · 07.*一次请求怎样死去/ })).toHaveAttribute('href', '/chapters/termination')
    expect(within(screen.getByRole('navigation', { name: '章节导航' })).getByRole('link', { name: /下一章 · 09.*优化收益去哪了/ })).toHaveAttribute('href', '/chapters/optimization')
  })

  it('renders chapter nine with the budget manifest and no unpublished next chapter', () => {
    renderRoute('/chapters/optimization')
    expect(screen.getByRole('heading', { level: 1, name: /优化收益\s*去哪了/ })).toBeInTheDocument()
    expect(screen.getByRole('complementary', { name: '第 09 章固定预算输入' })).toBeInTheDocument()
    expect(screen.getByRole('navigation', { name: '本章内容' })).toBeInTheDocument()
    expect(document.querySelectorAll('.chapter-prose > h2')).toHaveLength(5)
    expect(document.querySelectorAll('.chapter-prose')).toHaveLength(1)
    expect(screen.getAllByRole('figure')).toHaveLength(2)
    const finalBodyHeading = screen.getByRole('heading', { level: 2, name: '归因是证据链，不是直觉' })
    const practiceHeading = screen.getByRole('heading', { level: 2, name: '先归因落空的收益，再重建归因链' })
    expect(finalBodyHeading.compareDocumentPosition(practiceHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
  ...
  it('renders chapter ten with the cache manifest and no unpublished next chapter', () => {
    ...
    expect(document.querySelectorAll('.chapter-prose')).toHaveLength(1)
    expect(screen.getAllByRole('figure')).toHaveLength(2)
    const finalBodyHeading = screen.getByRole('heading', { level: 2, name: '组合之前，先重签合同' })
    const practiceHeading = screen.getByRole('heading', { level: 2, name: '先判断共享块的归宿，再重建生命周期' })
    expect(finalBodyHeading.compareDocumentPosition(practiceHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    expect(screen.getByRole('heading', { level: 2, name: '用毛边前缀验收缓存账本' })).toBeInTheDocument()
    expect(screen.queryByText(/学习进度|掌握率/)).not.toBeInTheDocument()
